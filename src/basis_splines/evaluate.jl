...
function fillΦ!(Φt::Matrix{T}, d::Matrix{T}, x::Vector{T}, t::CardinalKnots{p}, Vp::Val) where {T, p}
    for i ∈ eachindex(x)
        xᵢ = x[i]
        k = find_k(t, xᵢ)
...
function fillΦ_coreKGP!(out::Matrix{T}, d::AbstractArray{T}, t::CardinalKnots{d}, l::Int, k::Int, x::T, ::Val{p}) where {T, p, d}
    begin
        denom = t.v * p
        α = (x - t[k-1]) / denom
        out[k-2,l] = 1-α
...
function fillΦ_core!(out::Matrix{T}, d::Matrix{T}, t::Knots{d}, l::Int, k::Int, x::T, ::Val{p}) where {T, p, d}
   @inbounds begin
